...
export class InscripcionService {
  ...
  async inscribir(dto: CrearInscripcionDto) {
    ...
    const existente = await this.inscripcionRepository.findByAlumnoYCurso(dto.alumnoId, dto.cursoId);
    if (existente) throw new ConflictException('El alumno ya está inscrito en este curso');

    return this.inscripcionRepository.create(dto);
  }

  async desinscribir(alumnoId: number, cursoId: number) {
    const inscripcion = await this.inscripcionRepository.findByAlumnoYCurso(alumnoId, cursoId);
