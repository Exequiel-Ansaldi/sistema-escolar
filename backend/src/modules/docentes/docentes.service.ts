...
export class DocentesService {
  ...
  async findAll(page = 1, limit = 10, search?: string) {
    return this.docentesRepository.findAll(page, limit, search);
  }
  ...
  async create(dto: CrearDocenteDto) {
    ...
    if (existenteEmail)
      throw new ConflictException('Ya existe un docente con ese email');
    const existenteDni = await this.docentesRepository.findByDni(dto.dni);
    if (existenteDni)
      throw new ConflictException(`Ya existe un docente con DNI ${dto.dni}`);
    if (new Date(dto.fechaIngreso) > new Date())
  ...
  async update(id: number, dto: ActualizarDocenteDto) {
    ...
    if (dto.dni) {
      ...
      if (existente && existente.id !== id)
        throw new ConflictException(`DNI ${dto.dni} ya registrado`);
    }
    if (dto.fechaIngreso && new Date(dto.fechaIngreso) > new Date())
      throw new ConflictException(
        'La fecha de ingreso no puede ser posterior a hoy',
      );
  ...
  async asignarMateria(dto: AsignarMateriaDto) {
    ...
    return this.docentesRepository.asignarMateria(dto.docenteId, dto.materiaId);
  }

  async quitarMateria(dto: AsignarMateriaDto) {
    return this.docentesRepository.quitarMateria(dto.docenteId, dto.materiaId);
  }
}
