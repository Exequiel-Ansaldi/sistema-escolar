    ...
    <div className="space-y-5 animate-fade-in">
      ...
      {showForm && (
        <FormModal title={editing?.id ? `Editar ${title}` : `Nuevo ${title}`} onClose={closeForm}>
          <form onSubmit={e => { e.preventDefault(); const fd = new FormData(e.currentTarget); const body: any = {}; fields.forEach(f => { const v = fd.get(f.key); if (f.type === 'number') body[f.key] = v ? Number(v) : undefined; else body[f.key] = v || undefined; }); save(body).then(row => { if (!row) return; try { onCreated?.(row); } catch (err) { console.error('Error en onCreated', err); } }); }}>
